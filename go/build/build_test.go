...
func TestLocalDirectory(t *testing.T) {
	...
	if p.ImportPath != "github.com/ggaaooppeenngg/gogo/go/build" {
		t.Fatalf("ImportPath=%q, want %q", p.ImportPath, "github.com/ggaaooppeenngg/gogo/go/build")
	}
}
...
func TestImportDirNotExist(t *testing.T) {
	...
	tests := []struct {
		...
		{"Import(full, 0)", "github.com/ggaaooppeenngg/gogo/go/build/doesnotexist", "", 0},
		{"Import(local, 0)", "./doesnotexist", filepath.Join(ctxt.GOROOT, "src/go/build"), 0},
		{"Import(full, FindOnly)", "github.com/ggaaooppeenngg/gogo/go/build/doesnotexist", "", FindOnly},
	...
	for _, GO111MODULE := range []string{"off", "on"} {
		t.Run("GO111MODULE="+GO111MODULE, func(t *testing.T) {
			...
			for _, test := range tests {
				...
				if p.ImportPath != "github.com/ggaaooppeenngg/gogo/go/build/doesnotexist" {
					t.Errorf(`%s got p.ImportPath: %q, want "github.com/ggaaooppeenngg/gogo/go/build/doesnotexist"`, test.label, p.ImportPath)
				}
			}
		})
	}
