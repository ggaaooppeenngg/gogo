...
import (
	...
	"github.com/ggaaooppeenngg/gogo/go/build/constraint"
	"github.com/ggaaooppeenngg/gogo/go/doc"
	exec "github.com/ggaaooppeenngg/gogo/go/internal/execabs"
	"github.com/ggaaooppeenngg/gogo/go/internal/goroot"
	"github.com/ggaaooppeenngg/gogo/go/internal/goversion"
...
func (ctxt *Context) importGo(p *Package, path, srcDir string, mode ImportMode) error {
	...
	if srcDir != "" {
		...
		if filepath.IsAbs(srcDir) {
			...
			return fmt.Errorf("github.com/ggaaooppeenngg/gogo/go/build: Dir is non-empty, so relative srcDir is not allowed: %v", srcDir)
		} else {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("github.com/ggaaooppeenngg/gogo/go/build: go list %s: %v\n%s\n", path, err, stderr.String())
	}

	f := strings.SplitN(stdout.String(), "\n", 5)
	if len(f) != 5 {
		return fmt.Errorf("github.com/ggaaooppeenngg/gogo/go/build: importGo %s: unexpected output:\n%s\n", path, stdout.String())
	}
	dir := f[0]
