...
import (
	"bytes"
	"fmt"
	"github.com/ggaaooppeenngg/gogo/go/internal/testenv"
	"os"
	"os/exec"
	"path/filepath"
	"runtime"
...
func TestImportTestdata(t *testing.T) {
	...
	if pkg := testPath(t, "./testdata/exports", tmpdir); pkg != nil {
		...
		for _, want := range []string{"github.com/ggaaooppeenngg/gogo/go/ast", "github.com/ggaaooppeenngg/gogo/go/token"} {
...
var importedObjectTests = []struct {
	...
	{"github.com/ggaaooppeenngg/gogo/go/ast.ObjKind", "type ObjKind int"},
	{"github.com/ggaaooppeenngg/gogo/go/types.Qualifier", "type Qualifier func(*Package) string"},
	{"github.com/ggaaooppeenngg/gogo/go/types.Comparable", "func Comparable(T Type) bool"},
	{"math.Pi", "const Pi untyped float"},
	{"math.Sin", "func Sin(x float64) float64"},
	{"github.com/ggaaooppeenngg/gogo/go/ast.NotNilFilter", "func NotNilFilter(_ string, v reflect.Value) bool"},
	{"github.com/ggaaooppeenngg/gogo/go/internal/gcimporter.FindPkg", "func FindPkg(path string, srcDir string) (filename string, id string)"},
	...
	{"github.com/ggaaooppeenngg/gogo/go/ast.Node", "type Node interface{End() go/token.Pos; Pos() go/token.Pos}"},
	// go/types.Type has grown much larger - excluded for now
	// {"github.com/ggaaooppeenngg/gogo/go/types.Type", "type Type interface{String() string; Underlying() Type}"},
}
...
func TestIssue13898(t *testing.T) {
	...
	fset := token.NewFileSet()
	imports := make(map[string]*types.Package)
	_, err := Import(fset, imports, "github.com/ggaaooppeenngg/gogo/go/internal/gcimporter", ".", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	for path, pkg := range imports {
		if path == "github.com/ggaaooppeenngg/gogo/go/types" {
			goTypesPkg = pkg
			break
		}
	}
	if goTypesPkg == nil {
		t.Fatal("github.com/ggaaooppeenngg/gogo/go/types not found")
	}
	...
	if !ok {
		t.Fatalf("github.com/ggaaooppeenngg/gogo/go/types.Object type is %v; wanted named type", typ)
	}

	// lookup go/types.Object.Pkg method
	m, index, indirect := types.LookupFieldOrMethod(typ, false, nil, "Pkg")
	if m == nil {
		t.Fatalf("github.com/ggaaooppeenngg/gogo/go/types.Object.Pkg not found (index = %v, indirect = %v)", index, indirect)
	}

	// the method must belong to go/types
	if m.Pkg().Path() != "github.com/ggaaooppeenngg/gogo/go/types" {
