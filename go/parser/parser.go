...
import (
	...
	"github.com/ggaaooppeenngg/gogo/go/scanner"
...
func assert(cond bool, msg string) {
	if !cond {
		panic("github.com/ggaaooppeenngg/gogo/go/parser internal error: " + msg)
	}
}
...
func (p *parser) parseGoGoStmt() ast.Stmt {
	if p.trace {
		defer un(trace(p, "GoGoStmt"))
	}
	pos := p.expect(token.GOGO)
	call := p.parseCallExpr("gogo")
	p.expectSemi()
	if call == nil {
		return &ast.BadStmt{From: pos, To: pos + 4} // len("gogo")
	}

	return &ast.GoGoStmt{GoGo: pos, Call: call}
}
