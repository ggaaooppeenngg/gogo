	...
	for _, decl := range file.Decls {
		...
		for _, spec := range gen.Specs {
			...
			if pkg, ok := importer.lookupPackage(path); ok {
				...
				fmt.Println("path", path, "fileDir", fileDir)
				pkg, err := importer.ImportFrom(path, fileDir, 0)
				if err != nil {
					fmt.Println("ImportFrom ", err)
					return err
				}
				scope := pkg.Scope()
				pname = pkg.Name()
	...
	for len(declsToDo) > 0 {
		...
		for i, decl := range declsToDo {
			...
			case *ast.FuncDecl:
				...
				println("func decl end")
			case *ast.GenDecl:
				switch decl.Tok {
				case token.IMPORT:
					println("import decl")
					decl.Specs = append(decl.Specs, &ast.ImportSpec{
						Name: &ast.Ident{Name: "runtime"},
						Path: &ast.BasicLit{Value: "\"github.com/ggaaooppeenngg/gogo/runtime\""},
					})
					println("import decl end")
				case token.TYPE:
...
func (t *translator) translateStmtgogo(ps *ast.Stmt) {
	if t.err != nil {
		return
	}
	if *ps == nil {
...
func (t *translator) translateBlockStmtgogo(pbs *ast.BlockStmt) {
	if pbs == nil {
		return
	}
	for i := range pbs.List {
		t.translateStmtgogo(&pbs.List[i])
	}
}

func (t *translator) translateFuncDeclgogo(pd *ast.Decl) {
	fd := (*pd).(*ast.FuncDecl)
	t.translateBlockStmtgogo(fd.Body)
}
...
func (t *translator) translateStmt(ps *ast.Stmt) {
	...
	case *ast.GoGoStmt:
		...
		e := ast.Expr(s.Call)
		t.translateExpr(&e)
		args := []ast.Expr{s.Call.Fun}
		args = append(args, s.Call.Args...)

		expr := &ast.CallExpr{Fun: &ast.Ident{Name: "runtime.NewProc"}, Args: args}
		exprStmt := ast.ExprStmt{X: expr}
		*ps = &exprStmt
		println("gogo stmt end")
	case *ast.GoStmt:
		e := ast.Expr(s.Call)
		t.translateExpr(&e)
		s.Call = e.(*ast.CallExpr)
